...
class Moderation(commands.Cog):
    ...
    async def kick(self, inter, пользователь: disnake.Member, причина):
        ...
        if пользователь.top_role >= inter.user.top_role:
            await inter.response.send_message(
                "Вы не можете кикнуть этого пользователя, так как у него/нее роль выше вашей.")
            return

        if пользователь.bot == True:
            await inter.response.send_message(
                "Вы не можете кикнуть бота. Если вам понадобилось его кикнуть, сделайте это вручную.")
            return
        ...
        try:
            if check_if_can(user=пользователь):
                ...
                embed.set_footer(text="©️ Mithic Vanilla. All rigts reserved",
                                 icon_url="https://cdn.discordapp.com/attachments/1105911982873378816/1222136596174733382/image.png?ex=661e58e6&is=660be3e6&hm=e99c9c3e143c41da23f5d07fd5b481c3e968c60c8de627bd859ec8cff4f6060f&")
                ...
                await пользователь.send(embeds=[embed], components=[
                    disnake.ui.Button(label="Наш сайт", style=disnake.ButtonStyle.link, url="https://mithic.ru/")])
            await пользователь.kick(reason=причина)
            await inter.response.send_message("Пользователь был кикнут с сервера")
        except Exception as ex:
            await inter.response.send_message(
                f"Произошла ошибка (код: ``{ex}``). Попробуйте снова. \nПри повторении ошибки обратитесь к `_thecoffee_`.",
                ephemeral=True)
    ...
    async def ban(self, inter, пользователь: disnake.Member, причина):
        ...
        if пользователь.top_role >= inter.user.top_role:
            await inter.response.send_message(
                "Вы не можете забанить этого пользователя, так как у него/нее роль выше вашей.")
            return

        if пользователь.bot == True:
            await inter.response.send_message(
                "Вы не можете забанить бота. Если вам понадобилось его забанить, сделайте это вручную.")
            return
    ...
    async def timeout(self, inter, пользователь: disnake.Member, причина, часы: int = 0, минуты: int = 0,
                      секунды: int = 1):
        if пользователь == inter.user:
        ...
        if пользователь.top_role >= inter.user.top_role:
            await inter.response.send_message(
                "Вы не можете замьютить этого пользователя, так как у него/нее роль выше вашей.")
            return
        ...
        try:
            if check_if_can(user=пользователь):
                ...
                embed.add_field(name="Длительность",
                                value=f"```Часы: {str(часы)},\nМинуты: {str(минуты)}\nСекунды: {str(секунды)}```",
                                inline=False)
                embed.add_field(name="Модератор", value=f"> ```{inter.user.name}```", inline=False)
                await пользователь.send(embeds=[embed], components=[
                    disnake.ui.Button(label="Наш сайт", style=disnake.ButtonStyle.link, url="https://mithic.ru/")])
            await пользователь.timeout(duration=datetime.timedelta(hours=часы, minutes=минуты, seconds=секунды),
                                       reason=причина)
            await inter.response.send_message(
                f"Пользователь был отправлен в мут на {часы} часов, {минуты} минут, {секунды} секунд")
        except Exception as ex:
            await inter.response.send_message(
                f"Произошла ошибка (код: ``{ex}``). Попробуйте снова. \nПри повторении ошибки обратитесь к `_thecoffee_`.",
                ephemeral=True)
    ...
    async def warn(self, inter, пользователь: disnake.Member, причина: str):
        ...
        if пользователь.top_role >= inter.user.top_role:
            await inter.response.send_message(
                "Вы не можете дать варн этому пользователю, так как у него/нее роль выше вашей.")
            return
        ...
        try:
            if check_if_can(user=пользователь):
                ...
                embed.add_field(name="Причина", value=f"> ```{причина}```", inline=False)
                embed.add_field(name="Модератор", value=f"> ```{inter.user.name}```", inline=False)
                embed.add_field(name="Всего у вас варнов", value=f"> ```{str(get_warn_value(пользователь.id))}```",
                                inline=False)
                await пользователь.send(embeds=[embed], components=[
                    disnake.ui.Button(label="Наш сайт", style=disnake.ButtonStyle.link, url="https://mithic.ru/")])
            ...
            if get_warn_value(пользователь.id) >= 3:
                if get_warn_value(пользователь.id) <= 6:
                    await inter.response.send_message(
                        f"Пользователю успешно выдан варн и он отправлен в мут на {str(get_warn_value(пользователь.id))} час(-ов)")
                    await self.bot.get_slash_command("admin silentmute").callback(self=self, inter=inter,
                                                                                  пользователь=пользователь,
                                                                                  причина="Автомод: за большое количество варнов",
                                                                                  часы=get_warn_value(пользователь.id))
                else:
                    await inter.response.send_message(f"Пользователю успешно выдан варн и он был забанен навсегда")
                    await self.bot.get_slash_command("admin silentban").callback(self=self, inter=inter,
                                                                                 пользователь=пользователь,
                                                                                 автомод=True)
            else:
    ...
    async def unwarn(self, inter, пользователь: disnake.Member, причина: str):
        ...
        if пользователь.top_role >= inter.user.top_role:
            await inter.response.send_message(
                "Вы не можете снять варн этому пользователю, так как у него/нее роль выше вашей.")
            return
    ...
    async def warns(self, inter, пользователь: disnake.Member):
        ...
        try:
            ...
            embed.set_footer(text="©️ Mithic Vanilla. All rigts reserved",
                             icon_url="https://cdn.discordapp.com/attachments/1105911982873378816/1222136596174733382/image.png?ex=661e58e6&is=660be3e6&hm=e99c9c3e143c41da23f5d07fd5b481c3e968c60c8de627bd859ec8cff4f6060f&")
            await inter.response.send_message(embeds=[embed], components=[
                disnake.ui.Button(label="Наш сайт", style=disnake.ButtonStyle.link, url="https://mithic.ru/")])
        except Exception as ex:
            await inter.response.send_message(
                f"Произошла ошибка (код: ``{ex}``). Попробуйте снова. \nПри повторении ошибки обратитесь к `_thecoffee_`.",
    ...
    async def silent_timeout(self, inter, пользователь: disnake.Member, причина, часы: int = 0, минуты: int = 0,
                             секунды: int = 1):
        try:
            await пользователь.timeout(duration=datetime.timedelta(hours=часы, minutes=минуты, seconds=секунды),
                                       reason=причина)
